...
3. The `pData`/`colData` data.frames of each `SummarizedExperiment` are stored
   in an SQLite table in an [entity-attribute-value][EAV] format in order to
   supoport covariates that are both common to all `SummarizedExperiment`s and
   unique to others.
...
Perhaps the most important file to be concerned with when creating a 
`FacileDataSet` is the `meta.yaml` file.

Upon `FacileDataSet` creation, the `meta.yaml` file can be used to correctly
encode the sample covariates from the `pData` into its appropriate value
representation in the EAV table. If one isn't provided, a default one will
be created on-the-fly.

After the FacileDataSet has been created, the information in the 
`sample_covariates` section of the `meta.yaml` is used to decode them back into
R.

yaml files are easily manipulated with your favorite text editor, so I
recommend you use the `metayaml_skeleton(se.list, ...)` function to first
create it. You can then modify it by hand, and finally use the hand tweaked
version of it in your call to `as.FacileDatSet(se.list, yaml, ...)`



## Well Groomed Datasets

TODO: Explain what I mean by a "well-groomed" list of `SummarizedExperiment`s.
...
single letter, or their caseb, are treated as being different.
