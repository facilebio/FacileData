...
I have converted each of the studies from `RangedSummarizedExperiment` to simply `SummarizedExperiment`as the former is not yet supported by `as.FacileDataSet`.

```{r load ep data}
library(ExpressionPlot)

ngs114 <- ep.project.from.id("PRJ0013167")
ngs171 <- ep.project.from.id("PRJ0013730")

ds <- list(
  NGS114 = as(ep.SummarizedExperiment(ngs114, attach.annot = TRUE), "SummarizedExperiment"),
  NGS171 = as(ep.SummarizedExperiment(ngs171, attach.annot = TRUE), "SummarizedExperiment")
)
```
