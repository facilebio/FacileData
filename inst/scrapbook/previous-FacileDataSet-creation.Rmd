The content here was taken from a pre-release version of the 
FacileDataSet-assembly.Rmd vignette.

I need to pick through the text below to see if there are informative tidbits
I missed in the new version of the FacileDataSet-assembly vignette before nuking
this text outright.

<!-- Old text below -------------------------------------------------------- -->

A FacileDataSet should be prepared as a *named* list of `SummarizedExperiment`s 
(or DGELists, ExpressionSets, etc). It is expected that these objects will share
some, but not necessarily all, of their sample annotation columns (`colData` or
`pData`). Columns with the same names should also have the same encoding, factor
levels, etc.
