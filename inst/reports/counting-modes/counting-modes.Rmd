...
```{r arg1-plotly, fig.width=4, fig.height=3}
gg <- ggplot(dat, aes(log2(count.u), log2(count.i))) +
  geom_point(aes(text=barcode, color=dataset)) +
  xlab("Union [log2(counts)]") +
  ylab("IntersectionStrict [log2(counts)]") +
  geom_abline(intercept=0, slope=1, color='red') +
  ggtitle("Union vs IntersectionStrict (ARG1)")
ggplotly(gg)
```
